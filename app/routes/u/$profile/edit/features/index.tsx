...
import type { FeaturesWithPostsIncluded } from "~/models/feature.server"
...
export const loader: LoaderFunction = async ({ params }) => {
    ...
    const features = await getAllFeaturesByUsername({
        profileUsername: params.profile
    })
    return json<FeaturesLoaderData>({ features });
}
            ...
            <div className="overflow-y-auto scrollbar-hide flex flex-col flex-1 py-4 divide-y divide-gray-200 dark:divide-gray-700">
                {
                    features.length > 0 ? (
                            ...
                            <div className="flex gap-9 justify-between py-4" key={id}>
                                ...
                                <div className="flex flex-col grow shrink-0 w-min">
                                    <div className="text-sm font-medium text-gray-700 dark:text-gray-200">
                                        {
                                            (url && published) ? (
                                                ...
                                                >
                                                    {title}{publisher && ` on ${publisher}`}                                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-3 h-3 inline-flex">
                                                        <path fillRule="evenodd" d="M5.22 14.78a.75.75 0 001.06 0l7.22-7.22v5.69a.75.75 0 001.5 0v-7.5a.75.75 0 00-.75-.75h-7.5a.75.75 0 000 1.5h5.69l-7.22 7.22a.75.75 0 000 1.06z" clipRule="evenodd" />
                                                    </svg>
                                                </a>
                                            ) : (
                                                <span className="mr-1">
                                                    {title}{publisher && ` on ${publisher}`}
                                                </span>
                                            )
                                        }
                                        {
                                    ...
                                    <div className="flex items-center flex-wrap gap-1 mt-4">
                                        <Link
                                            to={`../posts/new?featureId=${id}&by=features`}
                                            className="
                                            flex-none
                        ...
                        <div className={
                            ...
                            <img src={EmptyFeature} alt="" width={200} height={200} className="dark:invert" />
                            <Link
                                to="new"
                                className="btn-secondary"
                            >
                                Add an article you were featured in
                            </Link>
                        </div>
                    )
                }
