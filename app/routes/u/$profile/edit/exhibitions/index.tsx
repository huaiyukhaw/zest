...
import { getAllExhibitionsByUsername } from "~/models/exhibition.server"
import type { ExhibitionsWithPostsIncluded } from "~/models/exhibition.server"
...
export type ExhibitionsLoaderData = {
    exhibitions: ExhibitionsWithPostsIncluded
}

export const loader: LoaderFunction = async ({ params }) => {
    if (!params.profile) throw new Error("Profile username not found")
    const exhibitions = await getAllExhibitionsByUsername({
        profileUsername: params.profile
    })
    return json<ExhibitionsLoaderData>({ exhibitions });
}

const ExhibitionsIndexPage = () => {
    const { exhibitions } = useLoaderData<ExhibitionsLoaderData>()
            ...
            <div className="overflow-y-auto scrollbar-hide flex flex-col flex-1 py-4 divide-y divide-gray-200 dark:divide-gray-700">
                {
                    exhibitions.length > 0 ? (
                            ...
                            <div className="flex gap-9 justify-between py-4" key={id}>
                                ...
                                <div className="flex flex-col grow shrink-0 w-min">
                                            ...
                                            <div className="flex flex-col gap-1 mt-4">
                                                {
                                                    posts.map(({ postId, post }) => (
                                                        ...
                                                        >
                                                            <Link
                                                                to={`../posts/${postId}?by=exhibitions`}
                                                                className="
                                                                    w-full
                                    ...
                                    <div className="flex items-center flex-wrap gap-1 mt-4">
                                        <Link
                                            to={`../posts/new?exhibitionId=${id}&by=exhibitions`}
                                            className="
                                            flex-none
                        ...
                        <div className={
                            ...
                            <img src={EmptyExhibition} alt="" width={200} height={200} className="dark:invert" />
                            <Link
                                to="new"
                                className="btn-secondary"
                            >
                                Add an exhibition you were in
                            </Link>
                        </div>
                    )
                }
