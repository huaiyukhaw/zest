...
import type { SpeakingWithPostsIncluded } from "~/models/speaking.server"
import { AlertDialog } from "~/components/radix"
import { EmptySpeaking } from "~/images/empty"
...
export type SpeakingLoaderData = {
    speaking: SpeakingWithPostsIncluded
}

export const loader: LoaderFunction = async ({ params }) => {
    if (!params.profile) throw new Error("Profile username not found")
    const speaking = await getAllSpeakingByUsername({
        profileUsername: params.profile
    })
    return json<SpeakingLoaderData>({ speaking });
}
            ...
            <div className="overflow-y-auto scrollbar-hide flex flex-col flex-1 py-4 divide-y divide-gray-200 dark:divide-gray-700">
                {
                    speaking.length > 0 ? (
                            ...
                            <div className="flex gap-9 justify-between py-4" key={id}>
                                ...
                                <div className="flex flex-col grow shrink-0 w-min">
                                    ...
                                    <div className="flex items-center flex-wrap gap-1 mt-4">
                                        <Link
                                            to={`../posts/new?speakingId=${id}&by=speaking`}
                                            className="
                                            flex-none
                        ...
                        <div className={
                            ...
                            <img src={EmptySpeaking} alt="" width={200} height={200} className="dark:invert" />
                            <Link
                                to="new"
                                className="btn-secondary"
                            >
...
export default SpeakingIndexPage